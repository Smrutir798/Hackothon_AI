...
def analyze_perishability(ingredients_list, extra_text=""):
    ...
    try:
        # Enforce JSON mode and provide a simpler prompt structure to avoid confusion
        response = ollama.chat(model='llama3', format='json', messages=[
            {'role': 'user', 'content': prompt},
        ])
        
        content = response['message']['content']
        print(f"DEBUG: RAW OLLAMA OUTPUT: {content}") # Log for debugging

        # Clean up commonly seen pollution even with json mode
        ...
        # Handle wrapped responses like {"ingredients": [...]}
        if isinstance(data, dict):
            # If the model returned a dict, try to find a list inside
            for key, value in data.items():
                if isinstance(value, list):
                    return value
            # If it's a single object dict representing one ingredient? Unlikely but possible
            return [data]
        ...
        # Log the raw content if possible to see what went wrong
...
def recommend_recipes_endpoint(request: RecipeRequest, current_user: Optional[User] = Depends(get_current_user)):
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
             # Helper lambda to pass the ingredients list
...
def get_recipe_details(recipe_id: int):
    ...
    # We don't have user ingredients here, so partial matching won't show what's owned vs missing
    # Pass empty list for user_ingredients
